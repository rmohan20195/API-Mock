...
{
    [TestFixture]
    public class CapitalPlansControllerTests
    {
        private ICapitalPlanService _service;
        private CapitalPlansController _controller;
        private IUnitOfWork _unitOfWork;

        [SetUp]
        public void Setup()
        {
            _service = Substitute.For<ICapitalPlanService>();
            _unitOfWork = Substitute.For<IUnitOfWork>();
            _controller = new CapitalPlansController(_service, _unitOfWork);
        }

        [Test]
        public void CreateController_OmittingCapitalPlanService_ThrowArgumentNullException()
        {

            //Act
            // ReSharper disable once ObjectCreationAsStatement
            var ex = Assert.Throws<ArgumentNullException>(() => new CapitalPlansController(null, _unitOfWork));

            //Assert
            StringAssert.Contains("capitalPlanService", ex.Message);
        }
        ...
        public void CreateController_OmittingUnitOfWork_ThrowArgumentNullException()
        {
            ...
            //Act
            // ReSharper disable once ObjectCreationAsStatement
            var ex = Assert.Throws<ArgumentNullException>(() => new CapitalPlansController(_service, null));
        ...
        public void Put_ProjectIdProjectInformationDTO_ReturnsNoContent()
        {
            ...
            var projectInformationDTO = new CapitalPlanDTO
            {
                ProjectId = id
            };
